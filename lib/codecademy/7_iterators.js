function iterators() {
  ...
  fruits.forEach(element => console.log(`I want to eat a ${element}`)); // the map method
  ...
  const anims = ['hippo', 'tiger', 'lion', 'seal', 'cheetah', 'monkey', 'salamander', 'elephant'];
  ...
  const nums = [1, 50, 75, 200, 350, 525, 1000]; //  Choose a method that will return undefined

  cities.forEach(city => console.log('Have you visited ' + city + '?')); // Choose a method that will return a new array

  const longCities = cities.filter(city => city.length > 7); // Choose a method that will return a single value
  ...
  const smallerNums = nums.map(num => num - 5); // Choose a method that will return a boolean value

  nums.some(num => num < 0); // review
  ...
  const iterChaining = testArray.map(word => {
    return word += 'test';
  }, 3).filter(word => word.includes('ot'));
